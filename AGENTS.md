...
- **主要技術スタック** (詳細は [ADR-0001](./docs/adr/0001-initial-architecture.md) を参照):
  - 言語 / ランタイム: TypeScript + Node.js 22 LTS (ESM)
  - パッケージマネージャ: pnpm
  - CLI フレームワーク: commander
  - テスト / Lint / Format: vitest / ESLint (typescript-eslint) / Prettier
...
依存のインストールは初回 / `package.json` 変更時に `pnpm install` を実行する。CI と同じコマンドが使えるように `package.json` の `scripts` を維持すること。

| 用途           | コマンド                                          |
| -------------- | ------------------------------------------------- |
| build          | `pnpm build`                                      |
| format         | `pnpm format` (チェックのみ: `pnpm format:check`) |
| lint           | `pnpm lint`                                       |
| unit-test      | `pnpm test` (watch モード: `pnpm test:watch`)     |
| e2e-test       | `TODO` (placeholder。後続 Issue で実装)           |
...
- **目的**: Philharmonic を **使う側** の視点から、インストール・設定・運用・トラブルシュートをタスク指向でまとめる
- **配置**: `docs/guide/<トピック>.md`。現在は `README.md` / `getting-started.md` / `configuration.md` / `operations.md` の 4 ファイル構成
- **執筆主体**: 利用体験に影響する変更を含む PR の中で、必要箇所を同時に更新する
- **書きかたのルール**:
  - **タスク指向で書く**: 「動かす」「設定する」「運用する」のような目的軸でまとめる。フィールド全表 / state machine / API 全定義などの真実は `docs/specs/` 側にしか書かない
  - **真実は `docs/specs/` に一本化する**: ガイドから spec へリンクで誘導するだけにし、フィールド一覧などは再記述しない (二重メンテで rot するため)
  - **README はガイドへの導線を維持する**: README はピッチ + 最短手順 + ガイドへのリンクに留め、詳細な解説は `docs/guide/` に書く
...
- [ ] 利用者体験に影響する変更なら、関連するユーザガイド (`docs/guide/`) を確認した
