...
`philharmonic.yaml` の default は `github.token_source: auto` なので、**`gh auth login` 済みなら追加設定は不要** です。Philharmonic が起動時に `gh auth token` を呼んで token を取得します。
...
> Philharmonic は `GITHUB_TOKEN` / `GH_TOKEN` を Orchestrator + Runner の env allowlist 経由で透過させ、agent (Claude Code + `gh` CLI) が Status 遷移 / `git push` / PR 作成 / Issue コメント投稿に使います。fine-grained PAT (対象リポジトリと Project に絞ったもの) を強く推奨します。

> token 文字列は **YAML に書きません** (誤 commit リスク回避)。設定できるのは取得元 (`env` / `gh` / `auto`) のみです。

## 4. 対象リポジトリで `philharmonic init` を実行する

Philharmonic を **動かしたい先のリポジトリのルート** で `philharmonic init` を実行すると、最小構成の `.philharmonic/philharmonic.yaml` が scaffold されます (Philharmonic 関連ファイルは `.philharmonic/` 配下に集約されます)。対話的に主要項目だけを訊き、残りは default で埋めた yaml にコメント化されたサンプルが同梱されるので「どこをいじれば何が変わるか」が yaml 内から発見できます。
...
| `permission_mode: bypass` を採用するか?                                           | Y/n。agent が `gh` / `git push` を実行して PR を作るために実用上必須              |
| `.philharmonic/WORKFLOW.md` を scaffold するか?                                   | y/N。後から手で書いてもよい                                                       |
| `.philharmonic/` 生成物 (`worktrees/` / `runs/` / `serve.lock`) を ignore するか? | Y/n。`.gitignore` が既に存在する場合のみ訊く。config 本体は commit 可能なまま残す |

> 生成 yaml には `github.token_source` (default `auto`) と `safety.allow_bypass_in_serve` (default `false`) もコメントで同梱されます。`bypass + serve` で起動するときは `safety.allow_bypass_in_serve: true` を有効化するか、env で `PHILHARMONIC_ALLOW_BYPASS_IN_SERVE=1` を設定してください。
...
> **既存ユーザー向けの移行**: 旧来 repo root 直下に置いていた `philharmonic.yaml` / `WORKFLOW.md` は当面そのまま動作しますが、起動時に warning が出ます。`mkdir -p .philharmonic && git mv philharmonic.yaml .philharmonic/philharmonic.yaml` (および `WORKFLOW.md` も同様) で移行してください。
...
> **`permission_mode` の注意**: agent が `gh` / `git push` を実行するには Bash tool が必要なため、`permission_mode: bypass` の設定が **実用上必須** です (`auto` では Bash tool が対話プロンプトになり、headless 環境では permission denied になります)。`bypass` は worktree 外への副作用リスクを伴うため、隔離環境前提で使ってください。

## 6. 任せたい Issue を Project に積む

Issue 本文は **自由フォーマット** で構いません (構造化セクション必須はありません)。Philharmonic は本文をそのまま agent に渡します。書きやすいガイドの一例:
...
> 先行 Issue が close されてから後続 Issue を dispatch したい場合は、Issue 本文に `Depends-On: #<番号>` 行を書けば順序を表現できます (詳細: [operations.md `依存関係付き Issue を運用する`](./operations.md#依存関係付き-issue-を運用する-dag-scheduling))。最初の 1 件には不要です。
...
成功時は agent が PR を立てて Status を `In Review` に遷移します。失敗時は agent が判断で Status を `Failed` に遷移し、必要に応じて Issue にコメントを残します。再実行は人手で `Failed → Todo` に戻すか、別 Issue で再起票してください (自動 retry はサポートしていません)。
