...
| **[operations.md](./operations.md)**           | CLI コマンド (`run` / `serve` / `projects` / `clean` / `init` / `dashboard`) の使い分け、構造化ログ、`.philharmonic/runs/`、Snapshot HTTP API、TUI dashboard、トラブルシュート (gh auth / scope / lock 等) |
...
> 先行 Issue が close されてから後続 Issue を dispatch する順序制約を表現したい場合は、Issue 本文に `Depends-On: #<番号>` 行を書けます (詳細: [operations.md `依存関係付き Issue を運用する`](./operations.md#依存関係付き-issue-を運用する-dag-scheduling))。
...
| **`philharmonic dashboard`** (任意) | Snapshot HTTP API を購読する read-only TUI。daemon の uptime / running runs / totals を見るときに別ターミナルから起動する                       |
