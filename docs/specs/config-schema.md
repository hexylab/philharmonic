...
| `retry.max_attempts`          | `integer (>= 0)`                         | no   | `3`                       | `philharmonic serve` daemon の loop で `Failed` を自動的に `Todo` に戻す最大回数。`0` で自動 retry を無効化する (`philharmonic run` の挙動に揃える) (詳細: [serve-daemon.md#自動-retry-22](./serve-daemon.md#自動-retry-22)) |
| `retry.max_backoff_ms`        | `integer (>= 1)`                         | no   | `600000` (10 分)          | exponential backoff の上限 (ミリ秒)。`backoff(attempt) = min(10_000ms × 2^(attempt-1), max_backoff_ms)` でクリップされる                                                                                                     |
...
| `retry.max_attempts`     | `3`                       | Symphony 相当の自動 retry 回数として 3 を採用。`0` で自動 retry を無効化できる (`philharmonic run` の挙動に揃える)                                                                    |
| `retry.max_backoff_ms`   | `600000` (10 分)          | `attempt = 7` で base が 640s に到達するため 10 分でクリップ。十分大きな上限で日本語の長時間 outage 中も無駄な再試行を抑える                                                          |
