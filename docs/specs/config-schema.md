...
| `polling.interval_ms`  | `integer (>= 1)`                         | no   | `30000`                   | `philharmonic serve` のポーリング間隔 (ミリ秒)。`polling` キー自体を省略しても、空オブジェクト `polling: {}` でも内側 default が補完される。詳細は [serve-daemon.md](./serve-daemon.md) を参照            |
...
| `polling.interval_ms`  | `30000` (30 秒)           | Issue #21 の Constraints「デフォルト 30s」に揃える。GitHub API の rate limit と人間の体感応答速度のバランス点                                   |
