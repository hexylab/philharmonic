...
| 用語                    | 意味                                                                                              |
| ----------------------- | ------------------------------------------------------------------------------------------------- |
| **Config File**         | 対象リポジトリの `.philharmonic/philharmonic.yaml` (既定)。CLI 引数で別パスを指定可能             |
| **Legacy Config File**  | 旧来の repo root 直下 `philharmonic.yaml` (#67 で `.philharmonic/` 配下に移行)。後方互換 fallback |
...
- `loadConfig(path?: string, options?: LoadConfigOptions): Promise<Config>` を `src/config/index.ts` から export する
- `path` 省略時は `path.resolve(process.cwd(), '.philharmonic/philharmonic.yaml')` を読み込む。`~` 展開等は行わない
- 新 default が不在で legacy `philharmonic.yaml` (repo root 直下) が存在する場合は legacy を読み込み、`options.onLegacyPathUsed` callback を 1 度だけ呼ぶ (#67 後方互換 fallback)。CLI 側で warning を 1 行出して、利用者に `.philharmonic/` への移行を促す
- `--config <path>` の明示指定時は legacy fallback の探索を行わない (typo を疑うべきため)
...
| `workflow_file`                  | `string` (空文字不可)                    | no   | `.philharmonic/WORKFLOW.md` | prompt テンプレートファイル (Liquid)。relative path は repo root 基準で解決する。default のときに不在なら legacy `WORKFLOW.md` (repo root 直下) を確認し、存在すれば warning を出して legacy を使う (#67)。最終的に存在しなければ `buildPrompt` にフォールバック (詳細: [workflow.md](./workflow.md)) |
...
| `terminal_statuses`              | `string[]` (要素は空文字不可)            | no   | `['Done']`                  | `philharmonic serve` 起動時 / `philharmonic clean-stale` で「対応 worktree を cleanup してよい terminal 状態」とみなす Status option 名の集合 (#89)。詳細: [stale-worktree-cleanup.md](./stale-worktree-cleanup.md)                                                                                   |
...
| `agent.max_retry_attempts`       | `integer (>= 0)`                         | no   | `5`                         | `philharmonic serve` の自動 retry queue (#84 / ADR-0008) で 1 つの Issue を retry する最大回数。`0` で機能 off。詳細: [retry-queue.md](./retry-queue.md)                                                                                                                                              |
| `agent.max_retry_backoff_ms`     | `integer (>= 1)`                         | no   | `300000` (5 分)             | retry queue の指数バックオフ `10s * 2^(attempt-1)` の clamp 上限 (ms)。`agent.max_retry_attempts == 0` のときは参照されない                                                                                                                                                                           |
...
| `github.token_source`            | `'env' \| 'gh' \| 'auto'`                | no   | `auto`                      | GitHub token の取得元 (#68)。`env` は `GITHUB_TOKEN` / `GH_TOKEN` を読む。`gh` は `gh auth token` を起動時に呼ぶ。`auto` は env を試して空なら `gh` に fallback。**token 文字列そのものは絶対に YAML に書かない** (token 用フィールドはスキーマに存在しない)                                          |
| `safety.allow_bypass_in_serve`   | `boolean`                                | no   | `false`                     | `permission_mode: bypass` で `philharmonic serve` を起動するための明示的 opt-in (#68)。`true` か env `PHILHARMONIC_ALLOW_BYPASS_IN_SERVE=1` のどちらかが必要 (両方 false なら起動拒否)。`philharmonic run` には影響しない                                                                             |
...
type Config = {
  ...
  github: {
    tokenSource: 'env' | 'gh' | 'auto';
  };
  safety: {
    allowBypassInServe: boolean;
  };
};
...
| `workflow_file`              | `.philharmonic/WORKFLOW.md`                                                | Philharmonic 関連ファイルを `.philharmonic/` 配下に集約する方針 (#67) に揃える。デフォルト名のときのみファイル不在を許容してフォールバックし、legacy `WORKFLOW.md` (repo root 直下) があれば warning 付きで採用する (詳細: [workflow.md](./workflow.md)) |
...
| `terminal_statuses`          | `['Done']`                                                                 | GitHub Projects v2 が新規 Project に初期生成する Status options の Done を踏襲。custom Status (例: `Archived` / `Closed`) を使う Project は個別に上書きする (#89)                                                                                        |
...
| `agent.max_retry_attempts`   | `5`                                                                        | Symphony の `retry_attempts` default に揃える。一過性の失敗を 5 回まで自動再 dispatch する (#84 / ADR-0008)                                                                                                                                              |
| `agent.max_retry_backoff_ms` | `300000` (5 分)                                                            | `10s * 2^(attempt-1)` の clamp 上限。attempt 6 で計算上 320s となるため 300s で頭打ちになる。Symphony の挙動と揃える                                                                                                                                     |
...
```ts
export function loadConfig(path?: string, options?: LoadConfigOptions): Promise<Config>;
...
1. `path` が明示指定されているときは `fs.readFile(path, 'utf8')` を試み、`ENOENT` なら `ConfigFileNotFoundError(path)` を throw する。legacy fallback は行わない (typo を疑うべきため)
2. `path` が省略されているときは以下を順に試す:
   1. `path.resolve(cwd, DEFAULT_CONFIG_FILE)` (`.philharmonic/philharmonic.yaml`)
   2. `path.resolve(cwd, LEGACY_CONFIG_FILE)` (repo root の `philharmonic.yaml`) — 採用したら `options.onLegacyPathUsed(legacyPath, defaultPath)` を 1 度だけ呼ぶ
   3. どちらも不在なら `ConfigFileNotFoundError(defaultPath)` を throw する (新パスを案内する)
3. その他 I/O エラーは原因をそのまま伝播させない方針で wrap して throw する
4. `js-yaml` の `load` でパース。`YAMLException` を catch して `ConfigParseError` (path / line / reason) に変換する
5. zod の `safeParse` で検証。`success: false` のとき `ConfigValidationError` (path / 各 issue の `path` と `message` を整形) を throw する
6. 検証成功時は transform 済みの `Config` を返す

### Legacy fallback 方針 (#67)

- Philharmonic 関連ファイルを `.philharmonic/` に集約する方針へ変更したため、既定パスは `.philharmonic/philharmonic.yaml` / `.philharmonic/WORKFLOW.md` に揃える
- 既存ユーザーが repo root に置いていた `philharmonic.yaml` / `WORKFLOW.md` を一度に動かさなくても済むように、新 default が不在のときだけ legacy パスを確認して採用する fallback を当面残す
- legacy 採用時は CLI 側で 1 行だけ warn ログ (`legacy ... を repo root から読み込みました。.philharmonic/ への移動を推奨します (#67)`) を出して移行を促す
- `--config <path>` で明示パスを渡したとき、および `workflow_file` を default 値以外で指定したときは legacy fallback を行わない (typo 検出を優先する)
- legacy 削除のタイミングは別 Issue で扱う
...
- token 系フィールドのスキーマ化 (token は環境変数または `gh auth` 経由のみ。詳細は #68 / [serve-daemon.md](./serve-daemon.md))
