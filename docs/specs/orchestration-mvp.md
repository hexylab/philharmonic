...
| **Run**          | 1 ターンの orchestration 実行単位。`run-id`(ULID 想定) で識別                           |
...
- Runner の起動は ADR-0001 で定めた `claude -p ... --output-format stream-json --cwd <worktree> --permission-mode <auto|bypass>` を用い、stream-json をローカルに永続化する
...
- 構成:
  - `metadata.json` — `run_id` / `issue_number` / `started_at` / `finished_at` / `status` (`success`/`failed`) / `failure_reason` / `total_cost_usd` / `branch` / `pr_number`
  - `stream.jsonl` — Runner の `--output-format stream-json` を行ごとに追記したもの
  - `summary.md` — Runner の最終 `result` event を Markdown に整形したもの (人間レビュー用)
...
- run-id の生成方式 (ULID / UUIDv7 / タイムスタンプ + Issue 番号) — 後続 Issue で確定。確定の際には Claude Code CLI の `--session-id` が要求するフォーマット (UUID 形式) との両立を併せて検証する
