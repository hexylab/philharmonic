...
GitHub Projects v2 上のアイテムを起点に Claude Code (headless mode) を分離 worktree で実行し、生成された差分を Pull Request として提出するまでの 1 ターン分の orchestration フローを定義する。本 spec は ADR-0001 で確定済みの技術判断を前提に、実行時の振る舞い・状態遷移・失敗時の扱いを明文化する。
...
常駐ポーリングデーモンとして `runOnce` を一定間隔で繰り返したい場合は `philharmonic serve` を使う。daemon の loop 制御 / signal handling / structured ログは [serve-daemon.md](./serve-daemon.md) を参照。本 spec の 1 ターン分の挙動 (state machine / failure handling / PR 生成) は serve でもそのまま再利用される。
...
- Runner の起動は ADR-0001 で定めた `claude -p ... --output-format stream-json --verbose <permission-flag>` を用い、worktree は subprocess の `cwd` で渡し、stream-json をローカルに永続化する (Claude Code CLI に `--cwd` フラグは存在しないため。詳細は [claude-runner.md](./claude-runner.md))。`<permission-flag>` は `permission_mode` に応じて `--permission-mode acceptEdits` (`auto`) または `--dangerously-skip-permissions` (`bypass`) のいずれかになる
...
- `philharmonic run` (1 ターン実行) では自動 retry / 自動 merge / 並列実行を行わない。`philharmonic serve` daemon の loop に限り、`Failed` を一定回数まで自動的に `Todo` に戻す自動 retry を実装する (#22。詳細は [serve-daemon.md#自動-retry-22](./serve-daemon.md#自動-retry-22))
...
- **セキュリティ**:
  ...
  - `permission_mode: bypass` を使う場合の副作用範囲はホストファイルシステム全体であることをユーザに明示警告する (Orchestrator が runner 起動前に WARN ログを 1 行出す。ADR-0001 / claude-runner.md 参照)
...
| 値            | 意味                                                                                        | 遷移方向                             |
| ------------- | ------------------------------------------------------------------------------------------- | ------------------------------------ |
| `Todo`        | 未着手。デフォルトの dispatch 対象 (`dispatch_statuses` で別 Status 名へ変更可能、#38)      | (初期) → `In Progress`               |
...
`philharmonic run` (1 ターン実行) では `Failed` から `Todo` への戻しは **手動運用** (自動 retry なし)。
`philharmonic serve` daemon は `retry.max_attempts` の範囲で自動的に `Failed` → `Todo` に戻す (#22)。
...
5. Prompt Construction
   ...
   5.2 WORKFLOW.md (Liquid テンプレート) があればテンプレートを render して prompt を組み立て、無ければ buildPrompt にフォールバックする (詳細: workflow.md / prompt-construction.md)
   5.3 安全制約フッタ (push しない / PR 作らない / token 期待しない / Conventional Commits) を Orchestrator が無条件で末尾に連結する
   5.4 必要に応じ <run-id>/prompt.md として保存 (デバッグ用)
...
7. Result Triage
   ...
   7.3 Runner stall                → 9. Failure (reason: stalled, #25)
   7.4 worktree に commit が 0 件  → 9. Failure (reason: no_changes)
   7.5 上記以外                    → 8. PR Submission
...
- 抽出条件 (AND):
  - Status が `philharmonic.yaml` の `dispatch_statuses` に含まれる (未指定時のデフォルトは `['Todo']`。`Ready for Agent` 等の独自 Status を dispatch 対象にしたい場合は本キーで指定する。詳細: [config-schema.md](./config-schema.md))
...
prompt は **WORKFLOW.md (Liquid テンプレート)** が存在すればそれを上位レイヤとして render し、無ければ以下のフォールバック構成 (下位レイヤ = `buildPrompt`) で組み立てる。テンプレート利用時は末尾に `## Orchestrator からの追加制約` セクションが Orchestrator により無条件に連結される (workflow.md 参照)。
...
- **Retry**:
  - `philharmonic run` (1 ターン実行) では **自動 retry を行わない**。同 Issue を再度実行したい場合は人間が `Failed` → `Todo` に戻す
  - `philharmonic serve` daemon は **`retry.max_attempts` の範囲で自動 retry を行う** (#22)。詳細は [serve-daemon.md#自動-retry-22](./serve-daemon.md#自動-retry-22)
...
1. **対応する open PR が存在するか確認**
   ...
   - slug の再計算は行わない (Issue title 編集後でも prefix で照合できる方が頑健)
   - 一致する PR が 1 つでもあれば **skip** (既に PR が出ているのに In Progress が残っているのは Status 遷移失敗等のエッジケース。再実行しない)
...
2. **対応 worktree (`<repo-root>/.philharmonic/worktrees/issue-<番号>`) が存在するか確認**
   - 存在する場合: **強制 reset 後に再実行**
     ...
     - `branch` には現在の Issue title から再計算した `feature/<番号>-<slug>` を渡す。後続の `createWorkspace({ reuse: false })` は branch 残存があると `branch_already_exists` で abort してしまうため、ここで必ず branch を消しておく必要がある
     - 続いて新規 `createWorkspace({ reuse: false, ... })` で `origin/main` から fresh worktree を再作成
   - 存在しない場合: **新規作成して再実行**
     - そのまま `createWorkspace({ reuse: false, ... })` を呼ぶ
   - 再開 (resume) は MVP では行わない (Claude Code session resume 連携は別 Issue。本 Issue では「強制 reset」を選択する)
3. **再実行 (`dispatchSelected`)**
   - `runOnce` の候補選択以降の処理 (workspace 作成 → prompt 構築 → runner → 差分検知 → push → PR → Status `In Review` 遷移) と同じパスを通す
   - Status は既に `In Progress` のため、Todo → In Progress 遷移はスキップする
   - 失敗時は通常の `runOnce` 同様 `markFailed` パスに乗せ、Issue 失敗コメント + Status `Failed` 遷移を行ったうえで **次の Item に進む** (recovery 全体は break しない)
...
| info  | `recovery dispatch success`      | `issueNumber`, `runId`, `prNumber`            | 1 件再実行が成功                                                |
| warn  | `recovery dispatch failed`       | `issueNumber`, `runId`, `reason`              | 1 件再実行が失敗 (markFailed 済み)                              |
| warn  | `recovery dispatch error`        | `issueNumber`, `error`                        | 想定外 throw (= bootstrap 系) → recovery を打ち切らず次 item へ |
...
- **個別 item の `dispatchSelected` 失敗**: 通常の Failure 処理 (Issue コメント + Status Failed) を経て次 item へ
- **AbortSignal による中断**: 現在処理中の item は完走 (途中強制終了は MVP 範囲外)。次 item には進まずに recovery を break し、その後の `serveLoop` も signal が aborted のため起動直後に終了する

#### MVP でやらないこと

- Claude Code session の resume (本 Issue は強制 reset で確定)
...
- **対象の限定**: `git worktree list --porcelain` を起点に、(a) workspace root 配下に位置し、かつ (b) ディレクトリ名が `issue-<番号>` パターンに一致するエントリのみを候補とする。`main` 等の主リポジトリ worktree や、`issue-*` 以外のサブディレクトリは候補にならない (構造的に保護される)。`git worktree list` には載っているがディレクトリが存在しない (path-missing) エントリは skip する (auto-prune は行わない)
...
- **`--dry-run`**: 削除対象を stdout に列挙するだけで、`cleanupWorkspace` を呼ばない。ファイルシステムは一切変更しない
- **削除処理**: 候補ごとに `WorkspaceManager.cleanupWorkspace({ taskKey, branch, deleteBranch })` を呼び、worktree (とローカルブランチ) を削除する。`deleteBranch` は worktree に紐づく branch が `feature/<issueNumber>-` 形式に一致したときのみ `true` とし、それ以外 (`main` を checkout しているなど想定外シナリオ・detached HEAD 含む) は `false` で main 等を構造的に保護する。`cleanupWorkspace` は内部で `git worktree remove --force` を使うため、未コミットの変更があっても worktree 自体は削除する (debug が必要なら `--dry-run` で先に確認する運用)
- **exit code**: 削除候補が 0 件、または全候補の削除が成功したときは exit 0。1 件でも失敗したときは stderr に詳細を出して exit 1
- **対象外**: `<repo-root>/.philharmonic/runs/<run-id>/` (Run Log) は本コマンドの対象外。Run Log の retention は MVP では実装しない
...
- `permission_mode` の追加ガード (`PHILHARMONIC_BYPASS=1` のような環境フラグ二重確認) を入れるか — Issue #29 で `auto` をデフォルトに確定済み。bypass の追加確認は MVP では config + 警告ログで十分とし、必要であれば後続 Issue で検討
- 同一 Issue の再実行ガード (既存 PR が open のときに再実行されたら何をするか)
- Recovery: 前回 run 以降に Issue title が変更されている場合の branch 名解決 (現状は再計算 slug を使うため、空振りのまま `git worktree remove --force` で worktree だけは消える。実体 branch が残存して次回の `createWorkspace` を阻害する可能性がある。`git worktree list --porcelain` から実 branch を引いて削除する案)
...
- 並列実行 (1 ターン 1 タスク)
- `philharmonic run` (1 ターン実行) の自動 retry / 自動再開
  (`philharmonic serve` daemon に限り `retry.max_attempts` の範囲で自動 retry する。#22 / [serve-daemon.md#自動-retry-22](./serve-daemon.md#自動-retry-22))
