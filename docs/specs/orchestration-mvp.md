...
- `philharmonic run` (1 ターン実行) では自動 retry / 並列実行を行わない。`philharmonic serve` daemon は並列 dispatch (#24) と起動時 recovery (#23) を持つが、自動 retry は本 ADR で撤廃 ([serve-daemon.md](./serve-daemon.md))
...
| `In Progress` | Agent が処理中                                                            | **Agent** (prompt 受領直後に flip)        |
| `In Review`   | PR 作成完了。人間レビュー待ち                                             | **Agent** (PR 作成成功後に flip)          |
| `Done`        | PR が main にマージ済み                                                   | 人間 (orchestrator も agent も駆動しない) |
| `Failed`      | 任意のフェーズで失敗。Issue に失敗コメントを残した後に到達                | **Agent** (失敗判断時に flip)             |
...
4. Prompt Construction
   ...
   4.2 WORKFLOW.md (Liquid テンプレート) があればテンプレートを render し、無ければ buildPrompt にフォールバック (詳細: workflow.md / prompt-construction.md)
...
3. **Orchestrator からの追加指示 (Definition of Done)** — 以下を Orchestrator 側で常に追加する:
   - 着手直後に Project Status を `In Progress` に遷移する (`gh project item-edit ...`)
   ...
   - PR 作成成功後に Project Status を `In Review` に遷移する
...
- **Retry**:
  - `philharmonic run` (1 ターン実行) も `philharmonic serve` も **自動 retry を行わない** (ADR-0005 で撤廃)
  - 同 Issue を再度実行したい場合は人間 or agent が `Failed → Todo` に手動で戻す
...
| Workspace 衝突                        | 同名 worktree / 同名ローカルブランチが既存                                       | Failure (reason: `workspace_conflict`)。worktree は保持                  |
...
- `philharmonic run` / `philharmonic serve` の自動 retry (本 ADR で撤廃)
