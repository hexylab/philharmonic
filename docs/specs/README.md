...
- [retry-queue.md](./retry-queue.md) — `philharmonic serve` の in-memory retry queue (失敗 / stalled run の指数バックオフ自動再 dispatch) 仕様 (#84, ADR-0008)
- [manual-retry.md](./manual-retry.md) — 手動再実行 CLI `philharmonic retry <issue-number>` の仕様 (#88, ADR-0005 fallback 経路)
- [stale-worktree-cleanup.md](./stale-worktree-cleanup.md) — `philharmonic serve` 起動時と `philharmonic clean-stale` CLI による terminal Issue worktree の安全な cleanup 仕様 (#89)
