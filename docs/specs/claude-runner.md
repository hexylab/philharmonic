...
- `--dangerously-skip-permissions` (bypass) は本 Issue では使わない。`auto` モードのみ実装する
...
- **セキュリティ**:
  ...
  - `--dangerously-skip-permissions` は本 Issue では使わない (Constraint)
...
| `permissionMode`    | `'auto'`            | no   | 既定 `'auto'`。`--permission-mode acceptEdits` にマップ (ADR-0001) |
...
- `bypass` permission mode (`--dangerously-skip-permissions`) の追加時期 — 後続 Issue で permission policy を ADR 化したのち

## MVP でやらないこと

- bypass permission mode のサポート (本 Issue Constraint)
- 自動 retry / resume
