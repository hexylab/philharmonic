...
- #62 — env allowlist に `GITHUB_TOKEN` / `GH_TOKEN` を追加し、agent が `gh` / `git push` を実行できるようにする
...
- 上位フロー: [orchestration-mvp.md](./orchestration-mvp.md) の「5. Runner Execution」「Failure / Timeout」「Run Log」
...
- Runner subprocess に渡す環境変数は **allowlist 方式** で絞る (#49)。許可した key 以外は通さない。詳細は「Runner subprocess の環境変数 (allowlist)」節 (ADR-0005 で `GITHUB_TOKEN` / `GH_TOKEN` を allowlist に追加。agent が `gh` / `git push` で利用するため)
...
- **セキュリティ**:
  ...
  - `serve` で `bypass` を使う場合は `safety.allow_bypass_in_serve: true` (推奨) または env `PHILHARMONIC_ALLOW_BYPASS_IN_SERVE=1` を必須にする bootstrap guard を CLI レイヤに置く (#68 で OR に拡張。詳細: [serve-daemon.md](./serve-daemon.md))
...
- GitHub 認証 (ADR-0005 で追加): `GITHUB_TOKEN`, `GH_TOKEN` — agent が `gh` / `git push` で利用
...
- GitHub Enterprise / Octokit: `GH_ENTERPRISE_TOKEN` / `OCTOKIT_*` (`GITHUB_TOKEN` / `GH_TOKEN` は ADR-0005 で allowlist に追加。agent 委譲のため)
